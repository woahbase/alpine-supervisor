...
# runtime flags
MOUNTFLAGS := # -v $(CURDIR)/config:/etc/supervisor.d
PORTFLAGS  := #
PUID       := $(shell id -u)
...
run : ## run service
	docker run --rm -it $(RUNFLAGS) $(IMAGETAG)
...
# gets latest release version from APKBUILD in APorts Repo
# $1 = package name e.g supervisor
# $2 = filename relative to current dir, defaults to VERSION
# $3 = branch e.g edge / v3.19-stable
# $4 = repository e.g. main / community / testing
# cache value in $(CURDIR)/VERSION
# and dont refetch if file newer than 8 hours
define get_aports_version
$(shell \
	URL=$(if $(1),https://git.alpinelinux.org/aports/plain/$(if $(4),$(4),main)/$(1)/APKBUILD$(if $(3),?h=$(3),),$(error package is not provided));
	OFILE=$(CURDIR)/$(if $(2),$(2),VERSION);
	if [ ! -f $${OFILE} ] \
	|| [ "$$(( ($$(date +%s) - $$(stat $${OFILE}  -c %Y)) / 3600 ))" -gt 8 ]; \
	then \
		curl -jsSL $${URL} 2>/dev/null \
		| awk '/pkgver=/{v=$$2} /pkgrel=/{r=$$2} END {print v"-r"r}' FS='=' \
		> $${OFILE}; \
	fi;
	cat $${OFILE};)
endef
